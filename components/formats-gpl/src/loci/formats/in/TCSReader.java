...
public class TCSReader extends FormatReader {
  ...
  protected void initFile(String id) throws FormatException, IOException {
    ...
    IFDList ifds = tiffParser.getIFDs(TiffParser.SubIFDSelection.EXCLUDE);
  ...
  private void groupFiles() throws FormatException, IOException {
    ...
    IFD ifd = p.getIFDs(TiffParser.SubIFDSelection.EXCLUDE).get(0);
    s.close();

    int expectedIFDCount = p.getIFDs(TiffParser.SubIFDSelection.EXCLUDE).size();
    ...
    for (String file : list) {
      ...
      if (tp.getIFDs(TiffParser.SubIFDSelection.EXCLUDE).size() != expectedIFDCount ||
    ...
    for (String file : files) {
      ...
      for (String tiff : tiffs) {
        ...
        ifd = parser.getIFDs(TiffParser.SubIFDSelection.EXCLUDE).get(0);
        s.close();
