 ...
 *       V is the tag value.  Order appears to be the context for the key name?
 *         0 = "Image"
 *         1 = "TOPO"
 *         2 = date, probably "%d.%m.%Y %H:%M:%S" in strftime(3) format 
 *         3 = filename (base name including .tiff extension); separate planes have _zn suffixes, tiling and other channels may have additional suffixes.
 *         4/5 = number of z planes?
 *         6+7 = scale factor x?
 *         8+9 = scale factor y?
 *         10 = scale units
 *         11 = scale factor z?
 *         12 = 0
 *       I is numeric.  Some type of checksum?  Or maybe a numeric identifier/key/enum?  It's certainly unique with the exception of being duplicated for z planes, below, though the values are identical so it's not possible to determine if it's a checksum or an identifier based on that.
 *       A is always zero.  Could it ever be non-zero?
 *    </Tags>
 *    <Scaling>
 *      Appears to duplicate the information in <Tags> for numbers 6,7,8,9,11.
 *      <Factor_n> Where n is 0, 1 or 2 (scale factor for x, y or z)?
 *      <Type_n> WHere n is 0, 1 oe 2 (scale type = 76 for all cases as for Tags)  Type_2 missing in Tags.
 *      <Tags> Tags block with no content except for <Unknown>.
 *    </Scaling>
 *    <zn>
 *      Numbered z planes.
 ...
 *      I values are unclear (checksums?) and A is always zero as before.
 *    </zn>
 *    
...
/**
 * ZeissAxioVisionReader is the file format reader for Zeiss AxioVision files and their
 * companion XML file.
 *
 * <dl><dt><b>Source code:</b></dt>
 * <dd><a href="http://trac.openmicroscopy.org.uk/ome/browser/bioformats.git/components/bio-formats/src/loci/formats/in/ZeissAxioVisionReader.java">Trac</a>,
 * <a href="http://git.openmicroscopy.org/?p=bioformats.git;a=blob;f=components/bio-formats/src/loci/formats/in/ZeissAxioVisionReader.java;hb=HEAD">Gitweb</a></dd></dl>
 *
 * @author Melissa Linkert melissa at glencoesoftware.com and Roger Leigh r.leigh at dundee.ac.uk
 */
public class ZeissAxioVisionReader extends FormatReader {
  ...
  public ZeissAxioVisionReader() {
    super("Zeiss AxioVision TIFF", new String[] {"tif", "tiff", "xml"});
